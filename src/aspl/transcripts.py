...
class Transcript:
    ...
    def splice_site_types(self):
        return itertools.cycle([events.SpliceSiteType.ACCEPTOR, events.SpliceSiteType.DONOR])
    ...
    def is_valid(self) -> bool:
        ...
        for s1, s2 in zip(self.splice_sites, self.splice_sites[1:]):
            result &= ((s1.coord <= s2.coord) & (s1.strand == events.Strand.PLUS)) | ((s1.coord >= s2.coord) & (s1.strand == events.Strand.MINUS))
        return result
