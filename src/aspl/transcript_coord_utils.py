...
def _cut_exon_twosites(
    e: events.Exon, s1: events.SpliceSite | None, s2: events.SpliceSite | None
):
...
def cut_transcript(
    ...
    s1: events.SpliceSite | None,
    s2: events.SpliceSite | None,
):
...
def insert_exon_into_transcript(t: transcripts.Transcript, e: events.Exon):
    ...
    return transcripts.Transcript(
        sorted(t.splice_sites + [e.siteB, e.siteC], key=sorting_key)
    )
